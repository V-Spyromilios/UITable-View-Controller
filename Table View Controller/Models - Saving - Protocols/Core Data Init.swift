...
func initCoreData(context: NSManagedObjectContext) {
	...
	for imageName in imageNames {
		guard let image = UIImage(named: imageName),
			...
			print("PANIC: failed to create image file or get image data for '\(imageName)'")
			continue
		}
		FileAssistant.shared.saveFlagPath(for: imageName, imagePath: imagePath)
	}
	...
	do {
		...
		userDefaults.set(true, forKey: "CountriesAreInitialised")
	} catch {
